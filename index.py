...
def processRecords(records):
    for r in records:
        ...
        if data['messageType'] == 'CONTROL_MESSAGE':
            ...
            joinedData = transformLogEvent(data['logEvents'][0])
...
def lambda_handler(event, context):
    ...
    if isSas:
        streamARN = event.get('sourceKinesisStreamArn', event['deliveryStreamArn'])
    else:
        streamARN = event['Records'][0]['eventSourceARN']

    region = str(streamARN.split(':')[3])
    streamName = str(streamARN.split('/')[1])
    ...
    if isSas:
        ...
        faux_id = 0
        for r_record in event['Records']:
            faux_id += 1
            r_record['kinesis']['recordId'] = f'INVALID_{faux_id}'
            records.append(r_record['kinesis'])

        records = list(processRecords(records))
    ...
    for idx, rec in enumerate(records):
        if isSas:
            ...
            originalRecord = event['Records'][idx]['kinesis']
